	...
	namespace Uniproc {
		...
		template<class Time, class IIP = Null_IIP<Time>> class State_space
		{
					...
					const Analysis_options& opts)
			{
				...
				//std::cout <<"start with the analysis" << '\n';
			...
			std::vector<std::size_t> get_reduced_job() const
			{

				return Job_removed_sets;
			}
...
//we move the index_of job to be the global function
...
// we do the similar Edge structure for the 'graph' we want to use for the backward tracking
			...
			struct EdgeCE {
				...
				EdgeCE(const Job<Time>* s, const State* src, const State* tgt,
					const Interval<Time>& fr)
				: scheduled(s)
				, source(src)
				, target(tgt)
			...
			const std::deque<EdgeCE>& get_ce_edges() const
			{
				return edges_ce;
			}
			...
			std::vector<std::size_t> Job_removed_sets;
...
//#ifdef CONFIG_COUNTER_EXAMPLE
			// for backward tracking
			std::deque<EdgeCE> edges_ce;
...
//#endif
			...
			void update_finish_times(const State& s, const Job<Time>& j, Interval<Time> range)
			{	
				auto rbounds = rta.find(j.get_id());
				if (rbounds == rta.end()) {
				...
				if (observed_deadline_miss == true){
									
					std::vector<std::size_t> Job_CE_sets;

					Job_CE_sets = s.get_CE_jobs();
		
					Job_set sched_jobs_CE;


					for(int i=0; i < (Job_CE_sets.size()); i++){
				   		sched_jobs_CE.add(Job_CE_sets.at(i));
				   	//std::cout << Job_CE_sets.at(i) << '\n';
				   }

				   	/*std::cout <<s.get_scheduled_jobs()<< '\n';
					std::cout <<"get_scheduled_jobs " << '\n';*/

					/*std::cout <<"new set job" <<  sched_jobs_non_CE << '\n'; */
				    Index_set sched_jobs_non_CE{s.get_scheduled_jobs(),sched_jobs_CE};



					for(int i=0; i < (sched_jobs_non_CE.full_size()); i++){
				   		if(sched_jobs_non_CE.contains(i)==true){
							Job_removed_sets.push_back(i);
				   		}
				   }
	...
	foreach_possbly_pending_job_until(cpju_macro_local_s, cpju_macro_local_j, (cpju_macro_local_until)) \
		if (cpju_macro_local_j->latest_arrival() <= (cpju_macro_local_until))
			...
			bool is_eligible_successor(const State &s, const Job<Time> &j)
			{
				...
				/*if (j.get_deadline_miss() ==true && !j.exceeds_deadline(s.latest_finish_time())) {
					return false;
				}*/
            ...
            // new_state is the function
			template <typename... Args>
			State& new_state(Args&&... args)
			{

                // add the new state in state deque
				states.emplace_back(std::forward<Args>(args)...);
                //s_ref is the end of the state
				State_ref s_ref = --states.end();
                
                // njobs is how many number of jobs have been scheduled
                ...
                //num_todo_queues contains all the state are yet to be explored

                // initiatial state, added to the todo que needs to be explored
                // idx is which todo queue i should put my new state in
				auto idx = njobs % num_todo_queues;
                // the todo queue of the idx i just used from the previous line, push_back to the state of queue to the todo queue, the state I just created.
				todo[idx].push_back(s_ref);
                // How to find the state in deque
...
//#if !defined CONFIG_COLLECT_SCHEDULE_GRAPH || !defined  CONFIG_COUNTER_EXAMPLE
				...
				const Interval<Time>& finish_range)
			{
				...
				update_finish_times(to, j, finish_range);
				// update statistics
				num_edges++;
#ifdef CONFIG_COLLECT_SCHEDULE_GRAPH
...
//#ifdef CONFIG_COUNTER_EXAMPLE
				edges_ce.emplace_back(&j, &from, &to, finish_range);
//#endif	
			...
			void schedule_job(const State &s, const Job<Time> &j)
			{
				...
				current_state_id = num_states; 

				const State& next =
					new_state(s, j, index_of(j),Job_CE_sets,current_state_id, 
			...
			void explore_naively()
			{
				...
				while (not_done() && !aborted) {
					...
					if (!found_at_least_one &&
						...
						observed_deadline_miss = true;
                        // print the deadline missed state scheduled jobs
                        
                        // std::cout <<"scheduled job list" <<  s.get_scheduled_jobs() << '\n';
			...
			/*void Collect_graph_Backward(){

				 const State_space<Time, IIP>& space

				for (auto e : space.get_ce_edges()) {

					std::cout <<"what is the e looks  like" <<  e << '\n';
			...
			void schedule(const State &s, const Job<Time> &j)
			{
				...
				//std::cout <<"current states num" <<  num_states<< '\n';
				unsigned int current_state_id;
				...
				if (j.get_deadline_miss() ==true && !j.exceeds_deadline(finish_range.until())) {
					return;
				}
				...
				auto k = s.next_key(j);
                // (find all the states have the equal key) 
				auto r = states_by_key.equal_range(k);
                // checking the r (state) is empty or not, if it is not empty we are trying to merge the state. s = <key,state>. The last state is <eom.eom>
				if (r.first != r.second) {
					Job_set sched_jobs{s.get_scheduled_jobs(), index_of(j)};
                    // all the states have the same key, looping
					for (auto it = r.first; it != r.second; it++) {
                        // assign the state to the pointer, found
						...
						// cannot merge if the deadline is missed
						if (j.exceeds_deadline(s.latest_finish_time()))
							continue;

						// great, we found a match and can merge the states
						found.update_finish_range(finish_range);
						
						found.update_CE_set(Job_CE_sets);

						process_new_edge(s, found, j, finish_range);


						//std::cout <<"what is the e looks  like" <<  found.get_state_ID() << '\n';
						return;
					}
				}
				...
				// a new state.
          		...
          		/*new added code from yimi*/

                if (s.latest_finish_time()< j.earliest_arrival()){
				std::vector<std::size_t> Job_CE_sets;
				Job_CE_sets.push_back(index_of(j));
...
//the import funtion for nptest.cpp, the nptest.cpp is calling the explore in line 92. auto space = space::explore(problem,opts);
			void explore()
			{
				make_initial_state();
				...
				while (not_done() && !aborted) {
					...
					//std::cout <<"is the second run?" <<  opts.second_run << '\n';
					...
					// Identify relevant interval for next job
					// relevant job buckets
					auto ts_min = s.earliest_finish_time(); //where is the function?
					auto rel_min = s.earliest_job_release();
					auto t_l = std::max(next_eligible_job_ready(s), s.latest_finish_time());
					...
					if (!found_at_least_one &&
						...
						// we want to find the deadline missed job
					}
