...
namespace NP {
	...
	struct Scheduling_problem {
		...
		// this is only for backward tracking added by yimi
		typedef typename std::vector<Back_track<Time>> Back_tracking;		
		...
		// (4) add for backtracking
		Back_tracking backtracks;
		...
		//constructor for backtracking
		Scheduling_problem(Workload jobs, Precedence_constraints dag,
		                   Abort_actions aborts,Back_tracking backtracks,
		                   unsigned int num_processors = 1)
		: num_processors(num_processors)
		, jobs(jobs)
		, dag(dag)
		...
		{
			//std::cout <<"I am in problem function"<< '\n';		
