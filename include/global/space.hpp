...
namespace NP {
	...
	namespace Global {
		...
		template<class Time> class State_space
		{
			...
			//This dummy function is for finding counter example in uni processor
			std::vector<std::size_t> get_reduced_job() const
			{

				/*std::cout << "sched jobs removed elements are : ";

				  	for(int i=0; i < (Job_removed_sets.size()); i++)
				   	std::cout << Job_removed_sets.at(i) << '\n';*/
			...
			const std::deque<Edge>& get_ce_edges() const
			{
				return edges;
			}
			...
			// the job_reomved_sets is un necessary for the multi cores
