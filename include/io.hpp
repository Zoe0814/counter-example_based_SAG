...
namespace NP {
		...
		template<class Time> Job<Time> parse_job(std::istream& in)
	{
		...
		return Job<Time>{jid, Interval<Time>{arr_min, arr_max},
						 Interval<Time>{cost_min, cost_max}, dl, prio, tid, dm};
		}
		else
	    {
	    in.exceptions(state_before);
	...
	{
		unsigned long tid, jid;
		Time trig_min, trig_max, cleanup_min, cleanup_max, dm;
	...
	std::vector<Abort_action<Time>> parse_abort_file(std::istream& in)
	{
		...
		template<class Time> Back_track<Time> parse_back_track(std::istream& in)
	{
		std::ios_base::iostate state_before = in.exceptions();
		in.exceptions(std::istream::failbit | std::istream::badbit);
	    ...
	    in.exceptions(state_before);
	  	...
	  	return Back_track<Time>{JobID{jid, tid},source, target, ls};
	}



	template<class Time>
	std::vector<Back_track<Time>> parse_backtrack_file(std::istream& in)
	{
		// first row contains a comment, just skip it
		...
		std::vector<Back_track<Time>> back_track;

		while (more_data(in)) {

			back_track.push_back(parse_back_track<Time>(in));
